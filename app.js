...
const text = [
  ...
  form.addEventListener('submit', function(e){
    e.preventDefault(); //pt ca forms sunt obisnuite default sa incarce pe un server, dar noi nu folosim server
    const value = parseInt(amount.value); //ca sa vedem ce amount avem introds in input + parseInt pt ca numarul ca nu imi apara ca string, ci ca number
...
if(isNaN(value) || value <= 0 || value > 9){
    result.innerHTML = `<p class="result">${text[random]}</p>` //daca userul nu introduce nicio valoare, ii vom da primul paragraf
} else{
